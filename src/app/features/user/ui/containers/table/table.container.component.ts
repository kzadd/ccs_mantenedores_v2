...
export class TableContainerComponent {
  ...
  headers: TableHeader<User>[] = [
    { key: 'id', label: '#' },
    { key: 'name', label: 'Nombre' }
  ]

  handleShow(item: User): void {
    this._router.navigate([FULL_ROUTE_PATHS.dashboard.user.show.replace(':id', item.id.toString())])
  }

  handleEdit(item: User): void {
    this._router.navigate([FULL_ROUTE_PATHS.dashboard.user.edit.replace(':id', item.id.toString())])
  }
}
