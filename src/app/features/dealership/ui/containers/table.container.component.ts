...
export class TableContainerComponent {
  ...
  headers: TableHeader<Dealership>[] = [
    { key: 'id', label: '#' },
    { key: 'name', label: 'Nombre' }
  ]

  handleShow(item: Dealership): void {
    this._router.navigate([
      FULL_ROUTE_PATHS.dashboard.dealership.show.replace(':id', item.id.toString())
    ])
  }

  handleEdit(item: Dealership): void {
    this._router.navigate([
      FULL_ROUTE_PATHS.dashboard.dealership.edit.replace(':id', item.id.toString())
    ])
  }
}
